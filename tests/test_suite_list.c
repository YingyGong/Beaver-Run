...
void test_list_remove_first() {
  ...
  // check if the removed element is indeed the first one
  vector_t *removed = list_remove(l, 0);
  assert(vec_equal(*(vector_t *)removed, (vector_t){1, 1}));
  free(removed);
...
void test_list_resize() {
  // create a new list_t with a capacity of 4
  list_t *l = list_init(4, free);
  assert(list_capacity(l) == 4);
  // Add elements until the initial capacity is reached
  for (int i = 0; i < 5; i++) {
    vector_t *v = malloc(sizeof(*v));
    v->x = v->y = i + 1;
    list_add(l, v);
  }
  assert(list_capacity(l) == 8);
  list_free(l);
}
...
int main(int argc, char *argv[]) {
  ...
  DO_TEST(test_list_remove_first)
